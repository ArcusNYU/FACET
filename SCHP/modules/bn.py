...
try:
    ...
    # Fallback when inplace_abn C++/CUDA extension (and ninja) is unavailable.
    # Keeps inference usable with standard PyTorch ops.
    ACT_RELU = "relu"
    ...
    def _apply_activation(x, activation, slope):
        ...
        if activation == ACT_LEAKY_RELU:
            return functional.leaky_relu(x, negative_slope=slope, inplace=True)
        if activation == ACT_ELU:
            return functional.elu(x, inplace=True)
        return x

    def _fallback_inplace_abn(
        x,
        weight,
        bias,
        running_mean,
        ...
        y = functional.batch_norm(
            x, running_mean, running_var, weight, bias, training, momentum, eps
        )
        y = _apply_activation(y, activation, slope)
        return y, running_mean, running_var

    inplace_abn = _fallback_inplace_abn
    inplace_abn_sync = _fallback_inplace_abn

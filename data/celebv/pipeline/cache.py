"""
CelebV-HQ Dataset Pipeline Stage 4: Latent cache (T5 caption embedding + Wan VAE video latent).
...
Reads {out_root}/index.jsonl produced by main.py. For each qualified clip:
  ...
  3. Encode caption with T5EncoderModel -> [L, 4096].
  4. Encode video with Wan2_2_VAE -> [48, T', H', W'] (already mean-std normalized).
  5. Save {tgt_latent, t5_emb} to {latent_cache_dir}/{cid}.pt atomically.

Resume: latent_root is scanned once for {cid}.pt; matching items drop from todo.
        Stale `.pt.tmp` from killed runs are swept. `--shard i/N` -> hash(cid)%N.

Wan weights layout (cfg.prepare.weight_dir / WAN2.2):
    models_t5_umt5-xxl-enc-bf16.pth, google/umt5-xxl/, Wan2.2_VAE.pth
"""
...
def read_index(path: Path) -> List[Dict[str, Any]]:
    """Read index.jsonl; dedupe by clip_id. (No `part` for CelebV.)"""
    if not path.exists():
...
def clip_dir(out_root: Path, cid: str) -> Path:
    return out_root / "clip" / cid


def latent_pt_path(latent_root: Path, cid: str) -> Path:
    """latent_root/{cid}.pt (flat; matches celebv.py _load_cache)."""
    return latent_root / f"{cid}.pt"


def scan_cached_cids(latent_root: Path) -> set[str]:
    """One-shot walk: collect every {cid}.pt already on disk."""
...
def encode_caption(text_encoder: T5EncoderModel,
    ...
    """Returns [L, 4096] in T5 dtype (bf16). Empty caption -> single space."""
    if not caption or not caption.strip():
        caption = " "
    with torch.no_grad():
        emb_list = text_encoder([caption], device)
...
def encode_video(vae: Wan2_2_VAE,
                 video: torch.Tensor,
                 device: torch.device) -> torch.Tensor:
    """video: [3,T,H,W] in [-1,1]. Returns [48, T', H', W'] float32 (mean-std normalized)."""
    with torch.no_grad():
...
# ============================================================
def main():
    p = argparse.ArgumentParser("Cache T5 caption embeddings + Wan VAE video latents (CelebV)")
    ...
    for it in tqdm(todo, desc="cache"):
        ...
        try:
            ...
            # NOTE: Celebv dataset has no caption, thus all embedding will be on "" (empty string)
            ...
            save_atomic(payload, latent_pt_path(latent_root, cid))
            n_ok += 1

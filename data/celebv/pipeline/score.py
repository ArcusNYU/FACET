...
L3 VlmFilter : Qwen3-VL-8B-Instruct judge. Prompt template loaded once from
               data/celebv/pipeline/prompt.txt; `{category}` is interpolated
...
def _extract_json(text: str) -> Dict[str, bool]:
    """Tolerant JSON extractor for VLM output. Defaults to a 'reject' pair on parse failure."""
    text = text.strip()
    ...
    if not m:
        return {"match": False, "occlusion": True}
    try:
...
class VlmFilter:
    ...
    Interface:
        ...
        d = vlm.judge(rgb_uint8, category="hair")
        accept = d["match"] and not d["occlusion"]
    """

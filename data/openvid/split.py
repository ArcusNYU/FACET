"""
OpenVid train/val splitter.

src:    
    {prepare.out_root}/index.jsonl     # produced by pipeline/main.py
    each line = {"clip_id", "part", "n_refs", "path", "_status": "ok"}
out:
    atomic write -> {split_dir}/{train,val}.jsonl

Usage:
    python -m data.openvid.split                       # default 0.1 val
    python -m data.openvid.split --val-ratio 0.05
    python -m data.openvid.split --no-stratify --seed 7
    python -m data.openvid.split --force               # overwrite existing
"""
...
def split_stratified_by_part(
    rows: List[Dict[str, Any]], val_ratio: float, rng: random.Random,
) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Per-part shuffle + per-part cut, then concatenate. Each part contributes
    floor(n_i * val_ratio) (>=1 when n_i > 0) to val.
    """
    groups: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for r in rows:
        groups[r.get("part", "_unknown")].append(r)
...
# ============================================================
def main():
    p = argparse.ArgumentParser("split openvid index.jsonl into train/val.jsonl")
    ...
    p.add_argument("--no-stratify", action="store_true",
                   help="disable per-part stratification (pure random shuffle)")
    ...
    if args.no_stratify:
        ...
        train, val = split_stratified_by_part(rows, args.val_ratio, rng)
        mode = "stratified-by-part"

    print(f"[split] mode={mode}  seed={args.seed}  val_ratio={args.val_ratio}")
    print(f"[split] train={len(train)}  val={len(val)}")

    # per-part breakdown for visibility
    train_by = defaultdict(int)
    val_by = defaultdict(int)
    for r in train:
        train_by[r.get("part", "_unknown")] += 1
    for r in val:
        val_by[r.get("part", "_unknown")] += 1
    all_parts = sorted(set(train_by) | set(val_by))
    for part in all_parts:
        t, v = train_by[part], val_by[part]
        tot = t + v
        ratio = v / tot if tot else 0.0
        print(f"        {part:>10}  train={t:>6}  val={v:>5}  val/total={ratio:.3f}")
